"""Restructured Main Program. 

    Motor control task now handles both encoders and both motors in one instance
    QTR Update task now handles both QTR sensors at once and returns just a composite position (s_pos = pos)
    Linefollower has been updated to work with fewer shares, no queues anywhere
    IMU and TOF untouched.
...
if __name__ == "__main__":
    ...
    motor_control_obj = ControlBothMotors(motorKp, motorKi, motorKd, leftRequestedSpeed, rightRequestedSpeed, s_leftDistanceTraveled=s_leftWheelPosition, 
                                             s_rightDistanceTraveled=s_rightWheelPosition)

    line_follower_obj = LineFollower(s_pos=s_pos, leftTargetSpeed=leftRequestedSpeed, rightTargetSpeed=rightRequestedSpeed, 
                                     leftDistanceTraveled=s_leftWheelPosition, rightDistanceTraveled=s_rightWheelPosition, heading=s_field_heading, s_tofdistance=s_TOF_Distance, linePosTarget_front=lineCenterPosition_front, linePosTarget_rear=lineCenterPosition_rear, centerSpeed=baseSpeed, time_scale=lineFollowerPeriod)
    ...
    qtr_updater_obj = UpdateQTRs(qtrfront=qtr_front, qtrrear=qtr_rear, s_pos=s_pos, linePosTarget_front=lineCenterPosition_front, 
                                 linePosTarget_rear=lineCenterPosition_rear)

    #garbage_collector_obj = GarbageCollector()

    motorControl = cotask.Task(motor_control_obj.run, name = "Motors", priority = 1, period = period_all, profile = True)
    ...
    qtrUpdate = cotask.Task(qtr_updater_obj.run, name = "QTRs", priority = 1, period = period_all*1.0, profile=True)
