...
class VL53L1X:
    ...
    def __init__(self, i2c, address=41):
        ...
        if model_id != 0xEA or module_type != 0xAA or mask_rev != 0x10: # 1X module_type = 0xCC, 3X module_type = 0xAA
    ...
    def _write_register(self, address, data, length=None):
        """Write length (or len(data)) bytes to a register address.

        Args:
            address (int): Register address to write to.
            data (Any): Values to write to the register address.
            length (int, optional): Number of bytes to be written if the whole data input should not be used. Defaults to None.
        """
        if length is None:
            length = len(data)
        self._i2c.writeto(self.i2c_device_address, struct.pack(">H", address) + data[:length])

    def _read_register(self, address, length=1):
        """Read length bytes from a register address and return them as a bytearray.

        Args:
            address int: Register address to read from.
            length (int, optional): How many bytes to read. Also sets the return length. Defaults to 1.

        Returns:
            bytearray: Data from register of length specified by args
        """
        data = bytearray(length)
        self._i2c.writeto(self.i2c_device_address, struct.pack(">H", address))
        self._i2c.readfrom_into(self.i2c_device_address, data)
        return data
    ...
    def set_address(self, new_address: int):
        ...
        self.i2c_device_address = new_address
